...
@section('content')
        ...
        <form method="POST" action="/admin/dashboard/store" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
    ...
    <p class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="file_input_help">SVG, PNG, JPG or GIF (MAX. 800x400px).</p>
            ...
            <div class="mb-4">
                <label for="title" class="block text-gray-700">Judul Info</label>
                <input type="text" id="title" name="judul_info" class="w-full px-4 py-2 mt-2 border rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-600" value="{{Session::get('judul_info')}}">
            </div>
            <div class="mb-4">
                <label for="description" class="block text-gray-700">Deskripsi</label>
                <textarea id="description" name="deskripsi" class="w-full px-4 py-2 mt-2 border rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-600" rows="4">{{Session::get('deskripsi')}}</textarea>
            </div>
